...
def get_red_only(img):
    ...
    red[red < 0] = 0
    red[red > 255] = 255
    red_only = np.uint8(red)
    return red_only

def get_red_only_rgb(img):
    r = np.int16((img[:, :, 0]))
    g = np.int16((img[:, :, 1]))
    b = np.int16((img[:, :, 1]))
    red = r - g - b
